%% MAIN DROWSINESS ANALYSIS PIPELINE (DEEP LEARNING) - NO ARTIFACT REMOVAL
% This script implements an end-to-end deep learning pipeline to detect
% driver drowsiness from continuous EEG recordings WITHOUT advanced artifact removal.
%
% Pipeline Steps:
...
%    and then strategically split into training (70%), validation (15%),
%    and testing (15%) sets.
% 5. Model Training & Evaluation (Step 4): A 1D Convolutional Neural
%    Network (CNN) is built, trained, and evaluated using these datasets.
%
% To Use:
...
% External dataset integration (DROZY)
include_drozy = false; % set false to skip
drozy_mat_path = fullfile(pwd, 'Drozy.mat');
...
% Optionally ingest DROZY raw trials as EEGLAB datasets BEFORE preprocessing/ICA
if include_drozy && exist(drozy_mat_path, 'file')
    try
        fprintf('Ingesting DROZY raw data as EEGLAB sessions...\n');
        drozy_sessions = ingest_drozy_as_eeglab(drozy_mat_path);
        fprintf('DROZY sessions created: %d\n', numel(drozy_sessions));
    catch ME
        warning('Failed to ingest DROZY as EEGLAB: %s', ME.message);
        drozy_sessions = {};
    end
else
    drozy_sessions = {};
...
for i = 1:length(subject_folders)
    ...
    for j = 1:length(files_to_process)
        ...
        % Basic Preprocessing ONLY (SKIPPING vEOG regression and ICA artifact removal)
...
% --- Also process DROZY sessions through the same preprocessing/ICA/windowing ---
if ~isempty(drozy_sessions)
    fprintf('\nProcessing DROZY sessions through preprocessing, ICA, and windowing...\n');
    for k = 1:numel(drozy_sessions)
        entry = drozy_sessions{k};
        EEG = entry.EEG;
        label = entry.label;
        subject_id = entry.subject_id;
        
        % Determine if visualization should be enabled for DROZY sessions
        should_visualize_drozy = enable_visualization && (k <= viz_subject_limit);
        
        % Step 1
        try
            EEG = step1_preprocess_data(EEG, low_cutoff_freq, high_cutoff_freq, downsample_rate, frontal_channels, ref_channels, should_visualize_drozy);
        catch ME
            warning('DROZY %s (%s) preprocessing failed: %s', subject_id, label, ME.message);
            continue;
        end
        % Step 2 (best-effort)
        try
        ...
        % Step 3 (always attempt)
        try
            [X, Y] = step3_prepare_sequence_data(EEG, label, window_length_sec, stride_seconds);
            subject_ids_for_file = repmat({subject_id}, length(Y), 1);
            ALL_X_data = [ALL_X_data; X];
            ALL_Y_data = [ALL_Y_data; Y];
            ALL_subject_ids = [ALL_subject_ids; subject_ids_for_file];
        catch ME
            warning('DROZY %s (%s) windowing failed: %s', subject_id, label, ME.message);
        end
    end
    fprintf('DROZY processing complete. Total windows now: %d\n', length(ALL_Y_data));
end
...
% --- 2. RECOMMENDATION 1: SUBJECT-BASED DATA SPLITTING ---
fprintf('--- Phase 2: Subject-Based Data Partitioning ---\n');

unique_subjects = unique(ALL_subject_ids);
fprintf('Total subjects found: %d\n', length(unique_subjects));
fprintf('Subjects: %s\n', strjoin(unique_subjects, ', '));

% Verify preprocessing (RECOMMENDATION 2)
...
% --- 3. RECOMMENDATION 3: Leave-One-Subject-Out (LOSO) Cross-Validation ---
...
for fold = 1:k_folds
    ...
    try
        % Export data for Python deep learning instead of training in MATLAB
...
fprintf('Data exported for %d folds to python_data/ directory\n', k_folds);
fprintf('\nNext steps:\n');
fprintf('1. Run the Python deep learning script: python step4_domain_adversarial_training.py\n');
fprintf('2. Python will train models with proper domain adversarial training\n');
fprintf('3. Results will be saved back to MATLAB format\n');

% Save export completion flag
save('python_data/export_complete.mat', 'k_folds', 'unique_subjects');

% Placeholder results (will be updated by Python)
